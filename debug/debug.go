...
func DrawCamera(c gfx.Canvas, cam, cam2 *gfx.Camera) {
	cam.State = gfx.NewState()
	cam.Shader = shader
	cam.State.FaceCulling = gfx.BackFaceCulling
	...
	if fov := cam.FOV(); fov == -1 {
		width := float32(cam.View().Dx())
		height := float32(cam.View().Dy())
		...
		ratio := float32(c.Bounds().Dx()) / float32(c.Bounds().Dy())
		fovRad := fov / 180 * math.Pi

		hNear := float32(2 * math.Tan(fovRad/2) * cam.Near())
		wNear := hNear * ratio

		hFar := float32(2 * math.Tan(fovRad/2) * cam.Far())
		wFar := hFar * ratio
	...
	cam.Meshes = []*gfx.Mesh{m}
	c.Draw(c.Bounds(), cam.Object, cam2)
}
